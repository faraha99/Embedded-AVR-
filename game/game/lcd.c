...
void LCD_displayCustumCharaterRowColumn(uint8_t row, uint8_t column, uint8_t *data)
{
	 //LCD_sendCommand(0x40 + row);
...
void LCD_displayreturnCustumCharaterRowColumn(uint8_t row, uint8_t column, uint8_t *data)
{
	//LCD_sendCommand(0x40 + row);
	LCD_backwardCursor(row,column);
	_delay_ms(100);
	for (uint8 i = 0; i < 8; i++)
	{
		LCD_displayCharacter(data[i]);
...
void LCD_backwardCursor(uint8 row,uint8 col)
{
	uint8 lcd_memory_address;
